...
const Player = () => {
    ...
    if (state.audio) {
      ...
      const id = setInterval(() => {
        setPlayerTime((prevTime) => {
          if (prevTime >= state.audio.time) {
            clearInterval(id);
            dispatch({ type: "SET_ACTION", payload: "pause" });
            return state.audio.time;
          }
          return prevTime + 1;
        });
      }, 1000);
  ...
  const setAudio = (c) => {
    dispatch({
      type: "SET_AUDIO",
      payload: { audio: state.musics[c], id: state.musics[c].id },
    });
  };
    ...
    <div className="fixed flex justify-between px-5 bottom-0 start-0 end-0 bg-black py-5">
      <div className="flex gap-4 items-center">
        {data?.images[0] ? (
          ...
          <img
            src={
              "https://img.freepik.com/free-vector/profile-music-notes-background_23-2147492175.jpg"
            }
            className="w-14 rounded h-14 object-contain"
            alt=""
          />

...
const Card = ({ card, circle }) => {
  ...
  const playHandler = () => {
    ...
    if (play) {
      if (state.audio_id == card.id) {
        ...
        dispatch({
          type: "SET_AUDIO",
          payload: { id: card.id, audio: randomOwn || randomMusic },
        });
  ...
  return (
    <div className="w-full card relative text-white hover:bg-gradient-to-t mt-2 p-3 rounded-lg from-[#2d2d2d] to-transparent">
      <button
        onClick={playHandler}
        className={`bg-[#1ED760] hover:bg-[#3BE477] hover:!scale-[105%] transform card-play w-12 rounded-full text-lg absolute top-1/2 end-4 h-12 justify-center items-center ${
          play && state.audio_id == card.id ? "flex" : "hidden"
        }`}
      >
        {play && state.audio_id == card.id ? (
          <i className="fa fa-pause text-xl text-black"></i>
        ) : (
      ...
      <Link to={"/"} className={`w-full overflow-hidden`}>
        <img
          ...
          } shadow-xl shadow-[#00000050] w-full object-cover`}
